...
# Add a column for 2009 unemployment rates by gender, age, and race
employment_2009 <- subset_2009_empl_data %>% 
  pivot_wider(names_from = empl_status,
              values_from = estimate) %>% 
  mutate(`Unemployment Rate` = (`Unemployed`/`Total Civilian Labor Force`) * 100) %>% 
  # create a new column that retains information on the ACS year
  mutate(year = 2009)

# restrict unemployment rate values to 2 decimal places
employment_2009[, 9] <- round(employment_2009[, 9], digits = 2)
...
# Add a column for 2019 unemployment rates by gender, age, and race
employment_2019 <- subset_2019_empl_data %>% 
  pivot_wider(names_from = empl_status,
              values_from = estimate) %>% 
  mutate(`Unemployment Rate` = (`Unemployed`/`Total Civilian Labor Force`) * 100) %>% 
  # create a new column that retains information on the ACS year
...
# restrict unemployment rate values to 2 decimal places
employment_2019[, 9] <- round(employment_2019[, 9], digits = 2)

#### Employment 2009 and 2019 data set merge ####
